...
//            int fare = Integer.parseInt(parts[5]);
            
            String result = DatabaseUtil.processBooking(username, movie, theater,date, time, tickets);
            sendMessage(result);
            
            if (result.startsWith("BOOKING_SUCCESS")) {
                Server.broadcast("UPDATE:" + theater + ":" + date + ":" + time, clientId);
            }
        } else {
